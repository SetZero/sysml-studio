...
public static class DiagramLayout
{
    ...
    /// <summary>
    /// Lays out one connected group with MSAGL, at the origin, reorders rows
    /// where that crosses fewer edges, and keeps the routes and label places
    /// MSAGL chose unless a row was reordered or a wide layer wrapped.
    /// </summary>
    private static void LayOutComponent(Diagram diagram, List<DiagramNode> component, Dictionary<DiagramEdge, LaidRoute> laid)
    {
        ...
        // MSAGL's routes only fit the boxes where it put them: a group whose
        // rows were reordered or wrapped is routed afresh.
        var reordered = LayerOrder.ReduceCrossings(component, [.. edges]);
        if (WrapWideLayers(component, [.. edges]) || reordered)
            return;
